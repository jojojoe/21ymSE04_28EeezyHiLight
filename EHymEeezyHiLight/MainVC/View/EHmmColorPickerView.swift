...
class EHmmColorPickerView: UIView {
    
    let colorSlider = GDColorSlider()
    var backBtnClickBlock: (()->Void)?
    var okBtnClickBlock: ((UIColor)->Void)?
...
//        contentV.layer.borderColor = UIColor.black.cgColor
        contentV.snp.makeConstraints {
            ...
            $0.top.equalTo(self.safeAreaLayoutGuide.snp.bottom).offset(-343)
        }
...
//        backBtn.layer.borderWidth = 1
        ...
        let leftX = (UIScreen.width - width) / 2
        let topY = (313 - height) / 2
        
        
        
        colorSlider.frame = CGRect(x: leftX, y: topY, width: width, height: height)
        colorSlider.colorImageName = "colorPicker"
        colorSlider.isShowContentIndicate = true
        colorSlider.isShowRightIndicate = false
        colorSlider.isShowBottomIndicate = false
        colorSlider.adhere(toSuperview: contentV)
        colorSlider.addTarget(self, action: #selector(colorSliderChange(sender: )), for: .touchUpInside)
    ...
    @objc func okBtnClick(sender: UIButton) {
        okBtnClickBlock?(currentColor)
    }
    
    @objc func colorSliderChange(sender: GDColorSlider) {
